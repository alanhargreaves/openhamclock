  ...
  app.get('/api/rig-bridge/download/:platform', (req, res) => {
    ...
    if (platform === 'windows') {
      const script = [
        ...
        'if exist "%RB_CFG%" (',
        "    for /f \"usebackq delims=\" %%X in (`powershell -NoProfile -Command \"try{$c=Get-Content '%RB_CFG%'|ConvertFrom-Json;$p=$c.port;if(-not $p){$p=5555};$s=if($c.tls.enabled){'https'}else{'http'};$b=$c.bindAddress;if([string]::IsNullOrEmpty($b)-or $b -eq '0.0.0.0'){$b='localhost'};$p.ToString()+'|'+$s+'|'+$b}catch{'5555|http|localhost'}\"`) do (",
