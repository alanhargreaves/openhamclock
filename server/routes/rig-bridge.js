  ...
  app.get('/api/rig-bridge/download/:platform', (req, res) => {
    ...
    if (platform === 'windows') {
      const script = [
        ...
        ')',
        'start /b node rig-bridge.js',
        '',
        ':waitloop',
        'timeout /t 1 >nul',
        'netstat -ano | findstr ":%RB_PORT% " | findstr "LISTENING" >nul',
        'if errorlevel 1 goto waitloop',
        '',
        'echo.',
        'echo ========================================================================',
        'echo   IMPORTANT: Did you read the setup instructions in rig-bridge/README.md?',
        'echo   It contains absolutely vital requirements for your specific radio.',
        'echo ========================================================================',
        'echo.',
        'set /p "ACK=>>> Press Enter to confirm and open the Setup UI in your browser <<< "',
        '',
        ...
        'timeout /t 2 >nul',
        'netstat -ano | findstr ":%RB_PORT% " | findstr "LISTENING" >nul',
        'if not errorlevel 1 goto nodeloopport',
      ].join('\r\n');
    ...
    const script = [
      ...
      'SETUP_URL=$(RB_CFG="$RB_CFG" node -e \'try{const c=require(process.env.RB_CFG);const p=c.port||5555;const s=c.tls&&c.tls.enabled;const b=c.bindAddress&&c.bindAddress!=="0.0.0.0"?c.bindAddress:"localhost";console.log((s?"https":"http")+"://"+b+":"+p)}catch(e){console.log("http://localhost:5555")}\' 2>/dev/null || echo "http://localhost:5555")',
      'RB_PORT=$(echo "$SETUP_URL" | grep -Eo "[0-9]+$")',
      '',
      'UPDATE_MODE=0',
      'while [[ $# -gt 0 ]]; do',
      '  case "$1" in',
      ...
      'while ! lsof -ti tcp:$RB_PORT > /dev/null 2>&1; do sleep 1; done',
      '',
      'echo ""',
      'echo "========================================================================"',
      'echo "  IMPORTANT: Did you read the setup instructions in rig-bridge/README.md?"',
      ...
      'echo "========================================================================"',
      'echo ""',
      'read -p ">>> Press Enter to confirm and open the Setup UI in your browser <<< " dummy',
      '',
